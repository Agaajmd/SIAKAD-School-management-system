...
export default function TeacherPiketPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const piketRes = await fetch(`/api/employee/piket?classId=${nextTeacher.homeroomClassId || ""}`, { cache: "no-store" })
      if (piketRes.ok) {
        const piketData = await piketRes.json()
        setPiketSchedules(Array.isArray(piketData.schedules) ? piketData.schedules : [])
      }
    }
    load().catch(() => {})
  }, [])
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingSchedule) {
        const res = await fetch("/api/employee/piket", {
          method: "PATCH",
        ...
        const data = await res.json()
        if (!res.ok) throw new Error(data?.error || "Gagal memperbarui jadwal piket")
        setPiketSchedules((prev) => prev.map((item) => (item.id === editingSchedule.id ? data.schedule : item)))
        ...
        const data = await res.json()
        if (!res.ok) throw new Error(data?.error || "Gagal menambah jadwal piket")
        setPiketSchedules((prev) => [...prev, data.schedule])
        toast.success("Jadwal piket berhasil ditambahkan")
      }
      setShowModal(false)
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Gagal menyimpan jadwal piket")
    } finally {
  ...
  const handleDelete = async (id: string) => {
    setIsSubmitting(true)
    try {
      const res = await fetch(`/api/employee/piket?id=${id}`, { method: "DELETE" })
      const data = await res.json()
      if (!res.ok) throw new Error(data?.error || "Gagal menghapus jadwal piket")
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <GlassButton type="button" onClick={handleOpenCreate} className="w-full sm:w-auto justify-center"><Plus className="w-4 h-4 mr-2" />Tambah Piket</GlassButton>
      </div>

      <div className="flex gap-2 overflow-x-auto pb-2">
        {DAYS.map((day) => (
          <button type="button" key={day} onClick={() => setSelectedDay(day)} className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${selectedDay === day ? "bg-purple-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
            {DAY_LABELS[day]}
          </button>
        ))}
      </div>
            ...
            {schedulesForDay.map((schedule) => (
              <div key={schedule.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex gap-2"><GlassButton type="button" size="sm" onClick={() => handleOpenEdit(schedule)}><Edit className="w-4 h-4" /></GlassButton><GlassButton type="button" size="sm" variant="danger" onClick={() => { void handleDelete(schedule.id) }} disabled={isSubmitting}><Trash2 className="w-4 h-4" /></GlassButton></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {DAYS.map((day) => (
                <button type="button" key={day} onClick={() => setForm({ ...form, day })} className={`px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${form.day === day ? "bg-blue-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                  {DAY_LABELS[day]}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="max-h-48 overflow-y-auto space-y-2 p-3 bg-slate-50 rounded-xl border border-slate-200">
              {classStudents.map((student) => (
                <button type="button" key={student.id} onClick={() => toggleStudent(student.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all ${form.studentIds.includes(student.id) ? "bg-blue-50 border-2 border-blue-500" : "bg-white border border-slate-200 hover:border-slate-300"}`}>
          ...
          <div className="flex gap-3 pt-3 border-t border-slate-100">
            <GlassButton type="button" variant="secondary" className="flex-1 justify-center" onClick={() => setShowModal(false)} disabled={isSubmitting}>Batal</GlassButton>
            <GlassButton type="button" className="flex-1 justify-center" onClick={() => { void handleSave() }} disabled={isSubmitting}><Save className="w-4 h-4 mr-2" />Simpan</GlassButton>
          </div>
        </div>
      </GlassModal>
    </div>
